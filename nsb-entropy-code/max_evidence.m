...
%             'did not converge. ']);
    ...
    while ~((abs(dB0) < abs(B0*precision)) | (counter>maxcounter));
      counter = counter + 1 ;   
      F   =  K1/B0 + digamma(B0) - digamma(B0+N);
...
%             int2str(counter) ' iterations.']);
    ...
    while ~((abs(dBcl) < abs(Bcl*precision)) | (counter>maxcounter));
      ...
      if(ints==1)		% integer counts
        F    =  1/K*sum(kx(ng1).*digamma(nx(ng1) + Bcl/K)) - K2/K*digamma(1+Bcl/K) + ...
            K1/Bcl + digamma(Bcl) - digamma(Bcl+N);
        dF   =  1/(K^2)*sum(kx(ng1).*polygamma(1, nx(ng1) + Bcl/K)) - ...
            K2/(K^2)*polygamma(1,1+Bcl/K) - K1/Bcl^2 + polygamma(1, Bcl) - ...
            polygamma(1, Bcl +N);
      else			% fractional counts
        F    =  1/K*sum(kx.*digamma(nx + Bcl/K)) - K1/K*digamma(Bcl/K) + ...
            digamma(Bcl) - digamma(Bcl+N);
      ...
      if(dBcl<0) 
            dBcl=0;
            warning('MAX_EVIDENCE: negative (unstable) change.');
            errcode=4;
      end
      Bcl = Bcl +  dBcl;
